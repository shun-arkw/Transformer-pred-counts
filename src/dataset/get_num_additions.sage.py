...
# This file was *autogenerated* from the file src/dataset/get_num_additions.sage
from sage.all_cmdline import *   # import sage library

_sage_const_123456789 = Integer(123456789); _sage_const_10 = Integer(10); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_10000 = Integer(10000); _sage_const_100 = Integer(100); _sage_const_3 = Integer(3); _sage_const_1000 = Integer(1000); _sage_const_2 = Integer(2)
import os
import time
import argparse
import numpy as np
...
def output_static(data_nadds, threshold, num_bins=_sage_const_10 ):
    nadds_static = {}
    ...
    ret = plt.hist(data_nadds, bins=num_bins, range=(int(_sage_const_0 ), threshold), density=False)  
    counts, bin_edges = ret[_sage_const_0 ], ret[_sage_const_1 ]
    counts = counts.astype(int)

    for i in range(num_bins):
        nadds_static[f'{bin_edges[i]} <= num_additions < {bin_edges[i+_sage_const_1 ]}'] = int(counts[i])
...
class BuildDataset():
    def __init__(self, tag, F_G_str_list, nvars, field, max_itrs, threshold, matrix_entry_lower_bound, matrix_entry_upper_bound, matrix_entry_distribution, lower_lim_for_num_tokens, upper_lim_for_num_tokens, num_matrices_per_F, n_jobs=-_sage_const_1 , ):
    ...
    def get_matrix_nadds(self, F_G_str):
        ...
        for i in range(self.max_itrs):
            ...
            if nadds <= self.threshold or i == self.max_itrs - _sage_const_1 :
                weight_matrix = weight_matrix.reshape(-_sage_const_1 )
                nadds_str = str(nadds)
                break
    ...
    def generate_weight_matrix(self):
        ...
        while True:
            if distribution == 'uniform':
                ...
                weight_matrix[_sage_const_0 , :] = np.abs(weight_matrix[_sage_const_0 , :])

            elif distribution == 'normal':
                weight_matrix = np.random.normal(_sage_const_0 , _sage_const_10 , size=(self.nvars, self.nvars))
                weight_matrix = weight_matrix.astype(int)
                weight_matrix[_sage_const_0 , :] = np.abs(weight_matrix[_sage_const_0 , :])

            elif distribution == 'grevlex':
                first_row = np.ones((_sage_const_1 , self.nvars), dtype=int)
                rows = np.eye(self.nvars, dtype=int)
                rows = np.flip(np.eye(self.nvars, dtype=int), axis=_sage_const_1 )
                rows = -rows
...
class Writer():
    ...
    def save_stats(self, filename, threshold, num_bins=_sage_const_10 ):
...
def get_parser():
    ...
    parser.add_argument("--buchberger_threshold", type=int, default=_sage_const_10000 )
    parser.add_argument("--max_itrs", type=int, default=_sage_const_100 )
    parser.add_argument("--field", type=str)
    parser.add_argument("--nvars", type=int, default=_sage_const_3 )
    parser.add_argument("--matrix_entry_lower_bound", type=int, default=-_sage_const_1000 )
    parser.add_argument("--matrix_entry_upper_bound", type=int, default=_sage_const_1000 )
    parser.add_argument("--matrix_entry_distribution", type=str, default='uniform')
    parser.add_argument("--num_bins", type=int, default=_sage_const_10 )
    parser.add_argument("--lower_lim_for_num_tokens", type=int, default=_sage_const_0 , help="このトークン数未満のデータは無視する")
    parser.add_argument("--upper_lim_for_num_tokens", type=int, default=None, help="このトークン数より大きいデータは無視する")
    parser.add_argument("--test_num_matrices_per_F", type=int, default=_sage_const_1 )
    parser.add_argument("--train_num_matrices_per_F", type=int, default=_sage_const_1 )
